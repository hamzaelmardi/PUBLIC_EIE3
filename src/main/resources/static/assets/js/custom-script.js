(function($) {
	...
	/*$('#changeColor .colorPalette').on('click', function(){
		var cl = $(this).data('color'),path,path_log;

		path = 'css/custom'+cl+'.css';
		path_log = '../css/custom'+cl+'.css';
		$('#color_switcher').attr('href', path);
		$('#color_switcher_log').attr('href', path_log);

	});*/
	/*End Change Color WebSite*/

	/*Change Size WebSite*/
	$('.flex_color .size_cog').on('click', function(){

		var  type = this.dataset.size,
			 ...
			 h4  = $('h4'),
			 h5  = $('h5'),
			 textLower  = $('.sec-title .lower-text')
		;

		var currentSize = '15px',
			 navSize = nav.css('font-size'),
			 pSize = p.css('font-size'),
			 h2Size = h2.css('font-size'),
			 h3Size = h3.css('font-size'),
			 h4Size = h4.css('font-size'),
			 h5Size = h5.css('font-size'),
			 textLowerSize = textLower.css('font-size');

		// Increase Font Size
		if(type =='plus'){
			navSize = parseFloat(navSize)*1.2;
			if(navSize>='33') return false;

			/*------------*/
			pSize = parseFloat(pSize)*1.2;
			h2Size = parseFloat(h2Size)*1.2;
			h3Size = parseFloat(h3Size)*1.2;
			h4Size = parseFloat(h4Size)*1.2;
			h5Size = parseFloat(h5Size)*1.2;
			textLowerSize = parseFloat(textLowerSize)*1.2;

			nav.css('font-size', navSize);
			p.css('font-size', pSize);
			if(h2Size<='55') h2.css('font-size', h2Size);
			if(h3Size<='55') h3.css('font-size', h3Size);
			h4.css('font-size', h4Size);
			h5.css('font-size', h5Size);
			textLower.css('font-size', textLowerSize);
		...
		// Decrease Font Size
		if(type =='moins'){
			navSize = parseFloat(navSize)*0.8;

			pSize = parseFloat(pSize)*0.8;
			h2Size = parseFloat(h2Size)*0.8;
			h3Size = parseFloat(h3Size)*0.8;
			h4Size = parseFloat(h4Size)*0.8;
			h5Size = parseFloat(h5Size)*0.8;
			textLowerSize = parseFloat(textLowerSize)*0.8;
	...
	/*End Change Size WebSite*/


	$(document)
		.on('change','#selectProfilInscrit',function () {
			var id = $(this).val();

			$('.info_profil').hide();
			if(id == 3 || id == 4) $('#info_profil_societe').show();
			else if(id == 5 || id == 6) $('#info_profil_traite_doss').show();
	...
	$('.search-togglers').on('click', function() {
...
/* ==========================================================================
 ...
 function frm_search(id){
	  ...
	  if($("#"+id+" #zone_Ajouter:visible").length!=0){
          $("#"+id+" #zone_Ajouter:visible").hide('slow');
        }
	  
      }
 function frm_Ajouter(id){
        if($("#"+id+" #zone_Ajouter:visible").length!=0){
          $("#"+id+" #zone_Ajouter:visible").hide('slow');
        }
        else
          $("#"+id+" #zone_Ajouter").show('slow');
	  if($("#"+id+" #zone_search:visible").length!=0){
          $("#"+id+" #zone_search:visible").hide('slow');
        }
      }